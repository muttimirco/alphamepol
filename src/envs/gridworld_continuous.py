...
class GridWorldContinuous(gym.Env):
    ...
    def render(self):
        if self.game_display is None:
            ...
            pygame.display.set_caption('GridWorld with Slope')
    ...
    def step(self, action):
        ...
        # The slope is taken from a normal distribution, whose parameters change according to the sampled configuration
        # Add only if the agent is in the middle corridors
        if -self.dim <= x <= 0:
            lower, upper = 0, self.max_delta
            mu, sigma = self.max_delta / 2, self.max_delta / 20
