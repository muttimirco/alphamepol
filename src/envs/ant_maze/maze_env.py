...
MODEL_DIR = 'src/envs/ant_maze/assets'
...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        if observe_blocks:
            ...
            self.num_features = 30 # 29 + 1 due to the timestep * 0.001
        ...
        self.MAZE_STRUCTURE = structure = maze_env_utils.construct_maze(maze_id=self._maze_id, structure_id=self._structure_id)
        self.elevated = any(-1 in row for row in structure)  # Elevate the maze to allow for falling.
        self.stairs = any('s' in row for row in structure)
        self.blocks = any(
        ...
        for i in range(len(structure)):
            for j in range(len(structure[0])):
                ...
                if struct == 1:  # Unmovable block.
                    ...
                    ET.SubElement(
                        ...
                        size="%f %f %f" % (0.5 * size_scaling,
                                            ...
                                            height / 2 * size_scaling),
                        type="box",
                        material="UnmovableBlock",
                        contype="1",
                        conaffinity="1",
                        rgba="0.4 0.4 0.4 1",
                    )
                elif struct == 2:  # Unmovable block, high = 0 of 2
                    # Offset all coordinates so that robot starts at the origin.
                    ET.SubElement(
                        worldbody, "geom",
                        name="block_%d_%d" % (i, j),
                ...
                elif struct == 3:  # Unmovable block, high = 1 of 2
                    # Offset all coordinates so that robot starts at the origin.
                    ET.SubElement(
                        worldbody, "geom",
                        name="block_%d_%d" % (i, j),
                ...
                elif struct == 4 or struct == 's':  # Unmovable block, high = 2 of 2
                    # Offset all coordinates so that robot starts at the origin.
                    ET.SubElement(
                        worldbody, "geom",
                        name="block_%d_%d" % (i, j),
                    ...
                    ET.SubElement(
                        ...
                        type="box",
                        material="",
                        mass="0.001" if falling else "0.00004",
                        contype="1",
                        conaffinity="1",
                        rgba="0.9 0.1 0.1 1"
                    )
        ...
        self.wrapped_env = model_cls(*args, file_path=file_path, structure_id=self._structure_id, **kwargs)
    ...
    def _find_robot(self):
        ...
        for i in range(len(structure)):
            for j in range(len(structure[0])):
                if structure[i][j] == 'r' or structure[i][j] == 's':
